...
oauth2_scheme = OAuth2AuthorizationCodeBearer(
    authorizationUrl=f"{KEYCLOAK_URL}/realms/{REALM_NAME}/protocol/openid-connect/auth?prompt=login",
...
app = FastAPI(
    swagger_ui_init_oauth={
        "clientId": "public-client",                 # <-- your Keycloak client id (NOT "account")
...
async def validate_token(token: str) -> TokenData:
    try:
        ...
        print()
        # print("header type:", type(headers))
        # print(headers)
        print()
        print()
        kid = headers.get("kid")
        ...
        print()
        print("key_data type:", type(key_data))
        # print(key_data)
        print()
        print()
        print()
        ...
        print("Public key datattype:", type(public_key))
        print(public_key)
        print()
        print()
        print()
        ...
        print()
        print()
        print("Token decoded and verified successfully")
...
# http://localhost:8080/realms/testing/protocol/openid-connect/userinfo , maybe use this
@app.get("/user/me")
...
@app.get("/developer", dependencies=[Depends(has_role(["developer", "admin"]))])
async def developer_endpoint():
    return {"items":list(developer_items_db.values())}

@app.post("/developer/items", dependencies=[Depends(has_role(["developer", "admin"]))])
async def create_developer_item(item: Item):
    if item.name in developer_items_db:
        raise HTTPException(status_code=400, detail="Item already exists")
    developer_items_db[item.name] = item
    return {"message": f"Developer item '{item.name}' created successfully", "item": item}

@app.get("/developer/items/{item_name}", dependencies=[Depends(has_role(["developer", "admin"]))])
async def get_developer_item(item_name: str):
    item = developer_items_db.get(item_name)
    if not item:
        raise HTTPException(status_code=404, detail="Item not found")
    return item

@app.put("/developer/items/{item_name}", dependencies=[Depends(has_role(["developer", "admin"]))])
async def update_developer_item(item_name: str, updated_item: Item):
    if item_name not in developer_items_db:
        raise HTTPException(status_code=404, detail="Item not found")
    developer_items_db[item_name] = updated_item
    return {"message": f"Developer item '{item_name}' updated successfully", "item": updated_item}

@app.delete("/developer/items/{item_name}", dependencies=[Depends(has_role(["developer", "admin"]))])
async def delete_developer_item(item_name: str):
    if item_name not in developer_items_db:
        raise HTTPException(status_code=404, detail="Item not found")
    del developer_items_db[item_name]
    return {"message": f"Developer item '{item_name}' deleted successfully"}
